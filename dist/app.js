...
app.get('/api/restaurants/:id/menu', async (req, res) => {
    try {
        ...
        const menuItems = await db.collection('menu_items').find({
            restaurantId: new mongodb_1.ObjectId(id)
        }).toArray();
        res.json({
            success: true,
            data: menuItems,
...
app.delete('/api/restaurants/:id', async (req, res) => {
    try {
        const db = (0, database_1.getDb)();
        const { id } = req.params;
        const result = await db.collection('restaurants').deleteOne({ _id: new mongodb_1.ObjectId(id) });
        ...
        await db.collection('menu_items').deleteMany({ restaurantId: new mongodb_1.ObjectId(id) });
        res.json({
            success: true,
...
app.delete('/api/menu-items/:id', async (req, res) => {
    try {
        const db = (0, database_1.getDb)();
        const { id } = req.params;
        const result = await db.collection('menu_items').deleteOne({ _id: new mongodb_1.ObjectId(id) });
        if (result.deletedCount === 0) {
            return res.status(404).json({
                success: false,
...
app.post('/api/rides', async (req, res) => {
    try {
        ...
        const vehicleDrivers = drivers[vehicleType];
        const fareRange = baseFares[vehicleType];
...
app.get('/api/rides/:id', async (req, res) => {
    try {
        const db = (0, database_1.getDb)();
        const { id } = req.params;
        const ride = await db.collection('rides').findOne({ _id: new mongodb_1.ObjectId(id) });
        if (!ride) {
            return res.status(404).json({
                success: false,
...
app.patch('/api/rides/:id/status', async (req, res) => {
    try {
        ...
        const result = await db.collection('rides').updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: { status, updatedAt: new Date() } });
        ...
        const updatedRide = await db.collection('rides').findOne({ _id: new mongodb_1.ObjectId(id) });
        res.json({
            success: true,
...
app.put('/api/raiders/:id', async (req, res) => {
    try {
        const db = (0, database_1.getDb)();
        const { id } = req.params;
        const result = await db.collection('raiders').updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: { ...req.body, lastActive: new Date() } });
        if (result.matchedCount === 0) {
            return res.status(404).json({
                success: false,
...
app.delete('/api/raiders/:id', async (req, res) => {
    try {
        const db = (0, database_1.getDb)();
        const { id } = req.params;
        const result = await db.collection('raiders').deleteOne({ _id: new mongodb_1.ObjectId(id) });
        if (result.deletedCount === 0) {
            return res.status(404).json({
                success: false,
                error: 'Raider not found'
